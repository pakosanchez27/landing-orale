                ...
                <div class="w-full grid grid-cols-1 gap-6">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
	                    ...
	                    <div class="flex flex-col gap-2">
                            ...
                            @foreach ($industrias as $industria)
                                <option value="{{ $industria->id }}" @selected(old('industria') == $industria->id)>
                                    {{ $industria->nombre }}
                                </option>
                            @endforeach
                        </select>
	                ...
	                <div class="flex flex-col gap-2">
	                    ...
	                    <textarea id="descripcion" name="descripcion" rows="5"
	                        class="w-full rounded-md border border-slate-300 px-4 py-3 text-sm focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-200"
	                        placeholder="Describe el demo">{{ old('descripcion') }}</textarea>
                        @error('descripcion')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="btn-primario">Guardar demo</button>
                    </div>
                </div>
            </form>
        </div>
